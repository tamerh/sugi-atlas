#!/usr/bin/env python3
"""Body-gate regression check. Snapshot dir = <dist>/snapshots/drug/.
...
slug = (drug.get("slug") if isinstance(drug, dict) else None) \
       or slugify(drug["name"] if isinstance(drug, dict) else str(drug))
dist_root = ctx["params"].get("dist_root") or "/data/sugi-atlas-dist"
snap_dir = body_gate.snap_dir_for(dist_root, "drug")
